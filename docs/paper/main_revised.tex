...
The fine-tuned EfficientNetB0 achieved the highest overall performance, with an AUC of \textcolor{red}{0.873 (95\% CI 0.819--0.921)}, compared with \textcolor{red}{0.801 (95\% CI 0.733--0.866)} for the frozen feature-extraction configuration (EfficientNet FE+XGB) and \textcolor{red}{0.773 (95\% CI 0.707--0.835)} for the radiomics pipeline. Improvements were observed across \textcolor{red}{most} evaluation metrics, particularly sensitivity: in the fine-tuned EfficientNet configuration, \textcolor{red}{81.8\% of cured \textbf{examinations}} were correctly identified, compared with \textcolor{red}{60.6\%} for FE+XGB and \textcolor{red}{54.5\%} for XGBoost. \textcolor{red}{All metrics reported in Table~\ref{tab:all_models} are computed at the level of individual MRI examinations (exam-level); subject-level (animal-level) metrics are discussed below. A DeLong test confirmed that the AUC superiority of EfficientNet FT over radiomics is statistically significant (Z\,=\,$-$4.97, $p < 0.001$), whereas the difference between EfficientNet FE+XGB and radiomics did not reach significance (Z\,=\,$-$1.31, $p = 0.19$).}

\textcolor{red}{The radiomics model achieved higher specificity (0.810 vs.\ 0.754 for FT) and higher PPV (0.450 vs.\ 0.486), reflecting a more conservative prediction tendency, while the fine-tuned DL model achieved substantially higher sensitivity (0.818 vs.\ 0.545) and NPV (0.936 vs.\ 0.862). The frozen feature-extraction configuration occupied an intermediate position, with specificity (0.853) comparable to radiomics but sensitivity (0.606) lower than FT.}
...
\hline
\end{tabular}
\caption{\textcolor{red}{Exam-level} performance comparison under GLOO cross-validation \textcolor{red}{(N=298 examinations, 10 animals)}. FE: frozen feature extractor; FT: fine-tuned. F1w/F1m not reported (not used as selection criteria). \textcolor{red}{Bootstrap 95\% CIs (10\,000 resamples): Radiomics AUC [0.707, 0.835]; FE+XGB AUC [0.733, 0.866]; FT AUC [0.819, 0.921]. DeLong test: FT vs.\ Radiomics $Z=-4.97$, $p<0.001$; FE+XGB vs.\ Radiomics $Z=-1.31$, $p=0.19$. EfficientNet FT is selected as the recommended model: despite EfficientNet FE achieving a similar AUC, FT provides the highest sensitivity (0.818) and NPV (0.936), which are the clinically most relevant metrics for identifying cured animals.}}
...
Among all evaluated architectures and training configurations, the fine-tuned EfficientNetB0 model trained exclusively on grayscale MRI images achieved the best overall performance, with \textcolor{red}{an AUC of 0.873 (95\% CI 0.819--0.921), an accuracy of 76.8\%, and a sensitivity of 81.8\%}.

\textcolor{red}{Performance improved consistently along the transfer-learning axis. Training from scratch without mask information performed worst (AUC 0.5331); adding binary tumor masks raised discrimination substantially (AUC 0.7699); initializing from ImageNet weights provided a further boost, with the strongest frozen feature-extraction configuration reaching an AUC of 0.801 (95\% CI 0.733--0.866); and end-to-end fine-tuning produced the most balanced classifier (AUC 0.873, sensitivity 0.818, NPV 0.936). This ordering indicates that both pre-trained representations and full fine-tuning were the main drivers of performance.}
...
EfficientNet FT      & 0.790 & 0.913 & 1.000 \\
\hline
\end{tabular}
}
