...
class TFRecordImageHandler:
    ...
    def _augment(self, image, group_name):
        # Apply augmentation: horizontal flipping and random brightness
    ...
    def _load_dataset(self):
        ...
        dataset = dataset.map(lambda img, lbl: (self._normalize(img), lbl), num_parallel_calls=tf.data.AUTOTUNE)
    ...
    def __len__(self):
        # Use cardinality to determine dataset size
        size = tf.data.TFRecordDataset(self.tfrecord_file).cardinality().numpy()
