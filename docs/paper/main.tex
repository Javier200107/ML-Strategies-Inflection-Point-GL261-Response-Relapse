...
% Revision markup: \hl{} = yellow highlight (new/changed text, soul pkg)
...
\affil[*]{\small Corresponding author: \texttt{alfredo.vellido@upc.edu}}
...
% --- ABSTRACT ---
\begin{abstract}
Glioblastoma (GB) is the most aggressive primary brain tumor, characterized by a poor prognosis, limited response to therapy, and high rates of recurrence. Early therapeutic response assessment is challenging due to phenomena such as pseudoresponse and pseudoprogression. This study explores the potential of advanced machine learning (ML) strategies to predict long-term therapy outcomes using longitudinal T2-weighted Magnetic Resonance Imaging (MRI) data from a preclinical GL261 glioblastoma mouse model, acquired prior and during treatment. We compare two distinct approaches: a classical pipeline based on radiomic features coupled with an XGBoost classifier, and a deep learning (DL) pipeline using a fine-tuned EfficientNetB0 model. Our results demonstrate that while the radiomics approach identifies interpretable imaging biomarkers and achieves good predictive performance \hl{(AUC $\approx$ 0.77, 95\% CI 0.70--0.83)}, the DL-based model outperforms it across \hl{most} evaluation metrics, reaching an AUC of 0.85 and a sensitivity of 0.91. The DL model shows better generalization across individual subjects, and \hl{discriminative performance improves progressively throughout the follow-up period for both approaches}. Interpretability analysis via Grad-CAM confirms that the DL model's predictions are driven by anatomically relevant features. These findings highlight the significant potential of DL, enhanced by transfer learning, to serve as a powerful non-invasive tool for the early prediction of treatment efficacy in glioblastoma, paving the way for more robust and personalized therapy monitoring.
\end{abstract}
...
The DL approach achieved the highest overall performance, obtaining an AUC of 0.8511 compared with \hl{0.770 (95\% CI 0.703--0.832)} for the radiomics pipeline. Improvements were observed across \hl{most} evaluation metrics, particularly sensitivity: in the fine-tuned EfficientNet configuration, \hl{90.9\% of cured \textbf{examinations}} were correctly identified, compared with \hl{54.6\%} for XGBoost. \hl{All metrics reported in Table~\ref{tab:all_models} are computed at the level of individual MRI examinations (exam-level); subject-level (animal-level) metrics are discussed below.}

\hl{The radiomics model achieved higher specificity (0.806) and a higher positive predictive value than the fine-tuned DL model (PPV: 0.444 versus 0.556 for EfficientNet FT), reflecting a tendency of the radiomics approach to predict relapse more conservatively. In contrast, the fine-tuned DL model maintained a more favorable sensitivity-specificity trade-off, with the highest sensitivity (0.909) and NPV (0.968) across all configurations.}
...
\hline
\end{tabular}
\caption{\hl{Exam-level} performance comparison under GLOO cross-validation \hl{(N=298 examinations, 10 animals)}. FE: frozen feature extractor; FT: fine-tuned. PPV and NPV denote the positive and negative predictive values. \hl{Radiomics 95\% bootstrap CIs (10\,000 resamples): AUC [0.703, 0.832], Sensitivity [0.421, 0.667], Specificity [0.755, 0.856], PPV [0.338, 0.554], Accuracy [0.698, 0.799]. The selected model for clinical interpretation is EfficientNet FT, which achieves the best sensitivity-specificity balance despite EfficientNet FE obtaining a marginally higher AUC (0.858 vs.\ 0.851).}}
...
The baseline configuration, trained on the full feature set, achieved high specificity but poor sensitivity, effectively defaulting to a relapse prediction under the strong class imbalance. Because correctly identifying cured animals was the primary objective, subsequent configurations targeted sensitivity while preserving specificity. \hl{In the final configuration, all feature selection steps (variance filter, Spearman correlation filter, Mann--Whitney $U$ test, and heuristic filter retaining only original image-derived features) were applied exclusively within the training partition of each GLOO fold, so that no information from the held-out test animal contaminated any preprocessing or selection decision. This procedure yielded an exam-level AUC of 0.770 (95\% CI: 0.703--0.832) and a sensitivity of 0.545. At the animal level, majority-vote aggregation across examinations per subject produced an AUC of 0.92 and a specificity of 1.00 (all five relapsing animals correctly identified), with two cured animals misclassified.}

Feature importance analysis consistently identified a small subset of descriptors as dominant contributors to classification decisions, including Elongation, Dependence Entropy, Run Entropy, Short Run High Gray Level Emphasis, and High Gray Level Run Emphasis. These features collectively describe tumor morphology, structural organization, and texture heterogeneity.
...
The repeated identification of these descriptors across the different feature-selection strategies supports their robustness within the present cohort. \hl{These longitudinal trajectories are presented as illustrative group-level tendencies within five animals per group and should not be interpreted as statistically established biomarkers; formal repeated-measures testing and independent validation in larger cohorts would be required to confirm them.} Additional low-dimensional visualizations of the selected radiomic feature space (t-SNE and UMAP projections) are provided in Supplementary Figure S2.

\subsection{Deep Learning Model Analysis}

Among all evaluated architectures and training configurations, the fine-tuned EfficientNetB0 model trained exclusively on grayscale MRI images achieved the best overall performance, with an AUC of 0.8511, an accuracy of 81.9\%, and a sensitivity exceeding 90\%.

Performance improved consistently along the transfer-learning axis. Training from scratch without mask information performed worst (AUC 0.5331); adding binary tumor masks raised discrimination substantially (AUC 0.7699); initializing from ImageNet weights provided a further boost, with the strongest frozen feature-extraction configuration reaching an AUC of 0.8583; and end-to-end fine-tuning produced the most balanced classifier (sensitivity 0.9091, NPV 0.9684). This ordering indicates that both pre-trained representations and full fine-tuning were the main drivers of performance.
...
\hl{Predictive accuracy varied throughout tumor evolution. Stratified analysis by examination time point (early: day $\leq$21, mid: day 22--31, late: day $>$31) revealed a clear improvement of discriminative performance over time for the radiomics pipeline: AUC 0.576 (early), 0.803 (mid), and 0.962 (late). These results indicate that while the radiomics model achieves good discrimination at later time points, its performance during early treatment is modest, and claims of reliable early prediction should be interpreted cautiously in light of this finding.}

\hl{For the radiomics pipeline, misclassifications were concentrated in the earliest study days (approximately days 9--21), especially for cured animals, whose imaging signatures became distinctive only as therapeutic effects accumulated at later time points. Relapsing cases were classified more accurately and more uniformly across the timeline. The DL model similarly showed improving performance over time, though its temporal profile could not be fully stratified in this analysis as per-fold temporal predictions were not saved for the DL pipeline.} The per-day distribution of classification accuracy and misclassified examinations is reported in Supplementary Figure S3.
...
\node[rad, below=5mm of r1] (r2) {Feature selection\\ variance, correlation, Mann--Whitney $U$};
\node[rad, below=5mm of r2] (r3) {Temporal feature engineering\\ lags, rolling statistics, slopes};
\node[rad, below=5mm of r3] (r4) {XGBoost classifier};
...
Although the dataset is longitudinal, complete temporal sequences were not modeled jointly as a multivariate time series \cite{Prinzi2024}. Instead, each imaging time point was treated as an individual labeled observation. Temporal information was incorporated in two complementary ways: (i) in the radiomics pipeline, through engineered temporal descriptors computed across consecutive examinations of the same animal (see below); and (ii) in the evaluation, through a subject-aware cross-validation scheme and a temporal analysis of predictive performance. This design reflects the heterogeneous and irregular sampling of the cohort, in which the number and timing of examinations vary substantially across subjects.
...
Fine-tuning replaced the original ImageNet classification head with a custom binary classification head discriminating between cured and relapsing mice. The last 50 layers of the backbone were unfrozen, the model accepted three-channel input images, and the classification head consisted of a global pooling layer, a dense layer, batch normalization, dropout, and a final sigmoid activation. Training used a binary cross-entropy loss and the Adam optimizer, with a batch size of 32 and random oversampling of the minority class applied only to the training partition within each fold to avoid information leakage. \hl{Each fold model was trained for a fixed 20 epochs with no early stopping and no model checkpoint selection based on validation performance; the weights at the final epoch were used for evaluation. Although the held-out test animal's data were passed as \texttt{validation\_data} during training for monitoring purposes only, they did not influence any model-selection or stopping decision.} Dropout regularization was used to reduce overfitting. The full fine-tuning configuration is summarized in Supplementary Table~S4.
...
J.G.: methodology, software, formal analysis, investigation, data curation, visualization, and writing of the original draft.\newline
A.P.C.: conceptualization, data acquisition, supervision, funding acquisition, and manuscript review and editing.\newline
A.V.: conceptualization, methodology, supervision, funding acquisition, and manuscript review and editing.
...
\textbf{Supplementary information} accompanies this manuscript, comprising Supplementary Methods, Supplementary Tables S1--S4, and Supplementary Figures S1--S5.
