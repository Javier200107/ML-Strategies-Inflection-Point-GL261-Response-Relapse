...
The DL approach achieved the highest overall performance, obtaining an AUC of 0.8511 compared with 0.770 (95\% CI 0.703--0.832) for the radiomics pipeline. Improvements were observed across most evaluation metrics, particularly sensitivity: in the fine-tuned EfficientNet configuration, 90.9\% of cured examinations were correctly identified, compared with 54.6\% for XGBoost. All metrics reported in Table~\ref{tab:all_models} are computed at the level of individual MRI examinations (exam-level); subject-level (animal-level) metrics are discussed below.
