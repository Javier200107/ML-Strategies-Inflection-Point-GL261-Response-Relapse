...
The DL approach achieved the highest overall performance, obtaining an Area Under the Receiver Operating Characteristic Curve (AUC) of 0.8511 compared with 0.7015 for the radiomics pipeline. Improvements were observed across nearly all evaluation metrics, particularly sensitivity, where EfficientNet correctly identified 90.9\% of cured animals compared with 51.5\% for XGBoost.

Although the radiomics model achieved slightly higher specificity (0.8879 versus 0.7931), its substantially lower sensitivity indicates a tendency to classify uncertain cases as relapsing. In contrast, the DL model maintained a more favorable balance between sensitivity and specificity, resulting in better discrimination performance.
...
\hline
\end{tabular}
\caption{Performance comparison of radiomics and DL configurations. FE: frozen feature extractor; FT: fine-tuned. F1w and F1m denote the weighted and macro F1-scores, respectively. PPV and NPV denote the positive and negative predictive values, respectively.}
...
The baseline configuration, trained on the full feature set, achieved high specificity (0.9494) and accuracy (0.8455) but poor sensitivity (0.3485), effectively defaulting to a relapse prediction under the strong class imbalance. Because correctly identifying cured animals was the primary objective, subsequent configurations targeted sensitivity while preserving specificity. Statistical filtering with the Mann--Whitney $U$ test yielded the best overall discrimination (AUC 0.7015, sensitivity 0.5152); introducing lag-based temporal descriptors for the top-ranked features maintained this balance, and an additional variance-, correlation- and significance-based filtering step reached the highest precision (PPV 0.5926). These gains, although modest in absolute terms, were consistent and biologically coherent within a small, highly imbalanced cohort.
...
The repeated identification of these descriptors across the different feature-selection strategies supports their robustness within the present cohort. Additional low-dimensional visualizations of the selected radiomic feature space (t-SNE and UMAP projections) are provided in Supplementary Figure S2.
...
Predictive accuracy varied throughout tumor evolution. Both approaches exhibited lower performance during the earliest imaging time points, particularly before completion of the second TMZ administration, when tumor appearance remains relatively similar between future cured and future relapsing animals.

For the radiomics pipeline, misclassifications were concentrated in the earliest study days (approximately days 9--15), especially for cured animals, whose imaging signatures became distinctive only as therapeutic effects accumulated at later time points. Relapsing cases, on the contrary, were classified accurately and more uniformly across the timeline.

Despite this challenge, the DL model maintained substantially greater stability than the radiomics pipeline, markedly flattening the early-window error peak and improving per-subject consistency, with most animals surpassing 60\% individual accuracy. This reduction of early-day vulnerability is particularly relevant clinically, as it enables more reliable prediction of treatment response at earlier stages. The per-day distribution of classification accuracy and misclassified examinations is reported in Supplementary Figure S3.
...
Although DL models are frequently criticized as ``black boxes'', the Grad-CAM analysis demonstrates that their decisions can be interpreted and that correct predictions are consistently associated with anatomically relevant attention. Conversely, misclassifications were associated with diffuse or anatomically irrelevant attention, suggesting that errors stem primarily from uncertainty in lesion localization rather than from a failure to learn discriminative features. From a translational standpoint, the ability of the DL model to maintain performance during early prediction windows is particularly relevant, since the main clinical value of a predictive biomarker lies precisely in providing reliable information before conventional radiological assessment becomes conclusive.
...
We have demonstrated that both radiomics and DL are viable strategies for predicting therapy outcomes in preclinical glioblastoma from longitudinal MRI. However, a DL approach based on fine-tuning a pre-trained CNN outperforms the classical radiomics pipeline in terms of accuracy, sensitivity, and temporal stability. The DL model's ability to identify early and subtle markers of treatment response, combined with methods for interpreting its decisions, makes it a highly promising tool for developing non-invasive biomarkers. This work lays a strong foundation for future research aimed at creating more reliable and personalized methods for monitoring glioblastoma treatment.
...
Given the extremely high dimensionality of the radiomic feature space relative to the number of available subjects, a rigorous multi-stage feature selection strategy was implemented. First, missing values were imputed using mean-value substitution, and features exhibiting near-zero variance (variance $<0.01$) were removed. Redundancy was then reduced through correlation filtering using Spearman rank correlation coefficients: when two features exhibited a correlation magnitude greater than 0.95, one of them was discarded. Subsequently, statistical filtering was performed using the Mann-Whitney U test, retaining only features with statistically significant differences between cured and relapsing mice ($p < 0.05$); this non-parametric test was selected because most radiomic variables deviated substantially from normality. Finally, all selected features were standardized using z-score normalization.
...
Fine-tuning replaced the original ImageNet classification head with a custom binary classification head discriminating between cured and relapsing mice. The last 50 layers of the backbone were unfrozen, the model accepted three-channel input images, and the classification head consisted of a global pooling layer, a dense layer, batch normalization, dropout, and a final sigmoid activation. Training used a binary cross-entropy loss and the Adam optimizer, with a batch size of 32 and random oversampling of the minority class applied only to the training partition within each fold to avoid information leakage. Dropout and early stopping were used for regularization, and the checkpoint corresponding to the best validation performance was retained. The full fine-tuning configuration is summarized in Supplementary Table~S4.
